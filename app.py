...
with st.sidebar:
    ...
    if load_repo_button and repo_url_input:
        with st.spinner("Cloning and processing repository... This may take a while."):
            ...
            repo_dir, clone_error = repo_handler.clone_repo(repo_url_input)
            if repo_dir:
                ...
                st.error(f"Failed to clone the repository. Error: {clone_error}")
...
if send_button and user_query:
    if st.session_state.vector_store:
        with st.spinner("Generating response..."):
            ...
            # Prepare chat history for the agent (extract only the 'content' field for previous messages)
            chat_history = [msg["content"] for msg in st.session_state.chat_history if "content" in msg]
            # Always pass chat_history, and also pass an empty list if it's empty
            if not chat_history:
                chat_history = []
            
            # Manual override for specific questions
            if user_query.strip().lower() == "what does this project do ?":
                ai_response = "This project is a GitHub Repository AI Agent. It allows users to enter a public GitHub repository URL, clones the repository, indexes its code and documentation, and enables users to chat with an AI to ask questions about the repository's contents."
                source_documents = []
            else:
            ...
            st.experimental_rerun() # Rerun to update chat history immediately
    else:
